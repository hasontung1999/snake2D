...
//<<<<<<<<<<<<<<<<<< RGBPixmap:: readPPMFile>>>>>>>>>>>>>
int RGBpixmap::readPPMFile(string fname)
{  // Read into memory an mRGB image from an uncompressed PPM file.
	// return 0 on failure, 1 on success
	inf.open(fname.c_str(), ios::in | ios::binary); //must read raw binary char's.
	if (!inf) { cout << " can't open file: " << fname << endl; return 0; }
	int  row, col, maxval;
	// Read header information
	char cch1, ch2, dumb[4];
	inf.get(cch1); inf.get(ch2);
	if (cch1 != 'P' || ch2 != '6')
	{
		cout << "Wrong Image Format. Should be ppm" << endl; inf.close(); return 0;
	}

	do { inf.get(cch1); } while (cch1 != '\n');    //finish first line P6
	do { inf.get(cch1); } while (cch1 != '\n');    //fnish comment line
	...
	inf >> dumb;                             //height
	row = atoi(dumb);
	do { inf.get(cch1); } while (cch1 != '\n');    //finish image size line
	inf >> dumb;                             //read max color value
	maxval = atoi(dumb);
	do { inf.get(cch1); } while (cch1 != '\n');    //finish color value line
	nRows = row; // set class's data members
	nCols = col;
	cout << "Rows=" << nRows << " " << "Columns=" << nCols << "    " << "Maxval=" << maxval << endl;
	//Read pixel values
	pixel = new mRGB[nRows * nCols]; //space for array in memory
	if (!pixel) return 0; // out of memory!
	long count = 0;
...
//<<<<<<<<<<<<<<<<RGBpixmap :: makeChkrbd <<<<<<<<<<<<<<<<<<<
int RGBpixmap::makeChkrbd(void)
{
	nRows = 256;
	nCols = 256;
...
//<<<<<<<<<<<<<<<<RGBpixmap :: setTextureMMP
void RGBpixmap::setTextureMMP(GLuint textureName)
{
	glBindTexture(GL_TEXTURE_2D, textureName);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, nCols, nRows, GL_RGB, GL_UNSIGNED_BYTE, pixel);
}